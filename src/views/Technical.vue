    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-cardbg backdrop-blur-xl p-6 rounded-2xl border border-slate-700/50 tech-card col-span-1 lg:col-span-2 group">
        <div class="flex items-center gap-3 mb-6">
          ...
          <h4 class="text-lg font-bold text-white tracking-wide">8. Movie Duration Trend (Average Minutes)</h4>
        </div>
      ...
      <div class="bg-cardbg backdrop-blur-xl p-6 rounded-2xl border border-slate-700/50 tech-card group">
        <div class="flex items-center gap-3 mb-6">
          ...
          <h4 class="text-lg font-bold text-white tracking-wide">11. Movie Duration Distribution (Minutes)</h4>
        </div>
      ...
      <div class="bg-cardbg backdrop-blur-xl p-6 rounded-2xl border border-slate-700/50 tech-card group">
        <div class="flex items-center gap-3 mb-6">
          ...
          <h4 class="text-lg font-bold text-white tracking-wide">9. Top 10 Most Prolific Actors</h4>
        </div>
...
import { OhVueIcon as VIcon } from 'oh-vue-icons' // Ensure OhVueIcon is imported for consistent use across app
...
const durationDistData = computed(() => {
  const bins = { 'Short (< 60m)': 0, 'Standard (60-120m)': 0, 'Long (120-180m)': 0, 'Epic (> 180m)': 0 } // English bins
  movies.value.forEach(m => {
    if (m.type === 'Movie') {
      const durMatch = String(m.duration).match(/(\d+)/)
      if (durMatch) {
        ...
        else if (dur <= 120) bins['Standard (60-120m)']++ // Combine standard ranges for better visualization
